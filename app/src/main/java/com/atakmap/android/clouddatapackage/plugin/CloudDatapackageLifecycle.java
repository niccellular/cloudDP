...
package com.atakmap.android.clouddatapackage.plugin;
...
import com.atakmap.android.clouddatapackage.CloudDatapackageMapComponent;
...
public class CloudDatapackageLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = "CloudDatapackageLifecycle";

    public static Activity activity;

    public CloudDatapackageLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        CloudDatapackageLifecycle.this.overlays
                .add(new CloudDatapackageMapComponent());

        // create components
        Iterator<MapComponent> iter = CloudDatapackageLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(CloudDatapackageLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        CloudDatapackageLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
